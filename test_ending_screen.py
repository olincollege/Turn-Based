...
def test_draw_end_screen():
    ...
    with patch("pygame.font.SysFont") as mock_sysfont:
        ...
        draw_end_screen(
            screen, screen_width, screen_height, black, white, winner=3
        )
        mock_font.render.assert_any_call(
            "Draw! Both players captured each other's bases.", True, white
        )

        # Test for winner = 0 (draw due to time running out)
        draw_end_screen(
            screen, screen_width, screen_height, black, white, winner=0
        )
        mock_font.render.assert_any_call("Draw! Time ran out!", True, white)

        # Test for winner = 1 (Player 1 wins)
        draw_end_screen(
            screen, screen_width, screen_height, black, white, winner=1
        )
        mock_font.render.assert_any_call("Winner: Player 1", True, white)

        # Test for winner = 2 (Player 2 wins)
        draw_end_screen(
            screen, screen_width, screen_height, black, white, winner=2
        )
