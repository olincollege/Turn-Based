...
def help_screen(screen, width, height):
    ...
    help_text = [
        "Game Rules",
        (
            "The game is played on a grid representing Olin buildings. Players"
            " take turns sending Oliners between buildings. Both players will"
            " make their choices"
        ),
        (
            "in how and where they move their Oliners before the actions of"
            " both players occur. This simultaneous game-dynamic ensures that"
            " players have to make"
        ),
        (
            "predictive moves based on their opponent. The goal is to control"
            " the opponent's home-base while still holding control of your own"
            " before time is up(5 minutes)."
        ),
        "",
        (
            "Players can send Oliners from one building to another by selecting"
            " the source and destination buildings and specifying the number of"
            " Oliners to send."
        ),
        (
            "It is only possible to send Oliners from one building to another"
            " if both buildings are connected. The game ends when one or both"
            " players capture"
        ),
        "each other's home-base, or when the time runs out.",
        (
            "If the amount of Oliners for both players, in one building, is"
            " equal, then the building is captured by the player who sent the"
            " Oliners last."
        ),
        "",
        "Game Controls",
        (
            "- To send Oliners, select the source building, then the"
            " destination building, and specify the number of Oliners to send."
        ),
        "- To select a building, click on it with the mouse.",
        (
            "- To select the number of Oliners to send, type in the number"
            " using the keyboard."
        ),
    ...
    while running:
        ...
        for i, line in enumerate(help_text):
            ...
            screen.blit(
                rendered_text, (25, y_offset)
            )  # Render text with padding
            y_offset += 35  # Move down for the next line
        font = get_font(30)
        title_text = font.render(
            "Press Spacebar to Go Back to Main Menu", True, white
        )
        title_rect = title_text.get_rect(center=(width // 2, height - 25))
...
def main_menu(screen, width, height, test_mode=False):
    ...
    buttons = {
        "Play": pygame.Rect(
            (width - button_width) // 2, 100, button_width, button_height
        ),
        "Help": pygame.Rect(
        ...
        "Quit": pygame.Rect(
            (width - button_width) // 2, 400, button_width, button_height
        ),
    }
    if not test_mode:
